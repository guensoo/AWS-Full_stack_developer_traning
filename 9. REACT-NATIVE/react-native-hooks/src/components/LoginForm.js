  ...
  switch (action.type) {
    ...
    case 'SUBMIT':
      if (state.email !== 'test@example.com') {
        return { ...state, errorMessage: '이메일이 다릅니다.' };
      }
      if (state.password !== '1234') {
        return { ...state, errorMessage: '비밀번호가 틀립니다.' };
      }
...
const LoginForm = () => {
  ...
  return(
    <View>
      ...
      :
      <View>
        <TextInput style={styles.input} placeholder="이메일 입력" value={state.email} onChangeText={(text) => dispatch({type: 'SET_EMAIL',payload: text})}/>
        <TextInput style={styles.input} placeholder="비밀번호 입력" value={state.password} onChangeText={(text) => dispatch({type: 'SET_PASSWORD',payload: text})}/>
        <Text style={styles.errorText}>Error : {state.errorMessage}</Text>
        <Button title="로그인" onPress={() => dispatch({type:'SUBMIT'})}/>
