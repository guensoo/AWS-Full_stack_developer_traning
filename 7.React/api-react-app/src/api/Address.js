...
const Address = () => {
  ...
  const handleComplete = (data) => {
    // 내부에서 data를 가지고 어떤 내용을 파싱할지는 자유
    let addr = '';
    let extraAddr = '';
    ...
    if(data.userSelectedType === 'R'){
      addr = data.roadAddress; // 도로명 주소 선택 시 도로명 주소 할당 해주기
    } else{
      addr = data.jibunAddress; // 지번 주소 선택 시 지번 할당 해주기
    }
  ...
  return (
      <div className='form=group'>
        <div className='form-row'>
          ...
          <input type='button' onClick={handelClick} value='우편번호 찾기'/>
        </div>
